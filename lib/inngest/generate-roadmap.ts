...
export const generateRoadmap = inngest.createFunction(
  ...
  async ({ event }) => {
    ...
    if (!product || !product.user) throw new Error("Product or user not found");
    ...
    for (const day of roadmap) {
      ...
      for (const task of day.tasks) {
        ...
        if (!taskId) {
          ...
          if (!feature) {
            // @ts-expect-error Property 'tasks' does not exist on type 'Feature'
          ...
          const newTask = await prisma.task.create({
            data: {
              title: task.title,
              completed: false,
              // @ts-expect-error Property 'feature' does not exist on type 'Task'
